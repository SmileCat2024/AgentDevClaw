...
- Never edit files under `dist/`, `node_modules/`, or any other build output. Hand-patching build output never affects an already-running Runtime (module cache) and is silently wiped by the next build; always fix `src/` and let `npm run build` produce the output.
...
- `verified` — some passed run's evidence (executed tools / denials / fired hooks) is attributed to this Feature. Each Feature keeps its own ledger; `verification` records the source runId, time, and coverage detail. A library Feature with no tools and no hooks can never earn direct evidence; when a passed run verifies a Feature that depends on it via `static inject`, it advances together (`verification.transitiveVia` lists the covering dependents), so a pure-logic Feature never needs a fake hook just to be observed.
