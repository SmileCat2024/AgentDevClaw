...
`@ToolUse` 和 `@StepFinish` 在单个 Feature 中各只有一个决策方法。组合规则时逐个短路：
...
跨 Feature 按注册顺序执行。第一个 `Approve` 或 `Deny` 停止后续决策 hook：
...
- `@CallStart` 改写输入时读 `ctx.agent.getUserInput()`；
- `@StepStart` 只注入本 step 必需信息；
- `@ToolUse` 可以标准化 `ctx.call.arguments`，工具仍要校验；
- `@ToolFinished` 只处理已经发生的结果；
