...
describe('Server module imports', () => {
  ...
  it('should import all standalone server modules without error', async () => {
    const modules = [
      ...
      '../server/thread-control/thread-rotation.js',
    ];
...
describe('Route registration smoke test', () => {
  ...
  it('setupGitRoutes should register 10 git endpoints', async () => {
    ...
    assert.ok(postRoutes.some(r => r.path === '/protoclaw/git/discard'));
    assert.ok(postRoutes.some(r => r.path === '/protoclaw/git/graph'));
    assert.ok(postRoutes.some(r => r.path === '/protoclaw/git/commit_files'));
    assert.ok(postRoutes.some(r => r.path === '/protoclaw/git/branches'));
    assert.ok(postRoutes.some(r => r.path === '/protoclaw/git/branch'));
    assert.ok(postRoutes.some(r => r.path === '/protoclaw/git/stash'));
  });
