...
1. `@ToolFinished` 观察相关工具是否改变状态；
2. `@StepFinish` 更新计数并决定是否继续；
3. `@StepStart` 在需要时注入一次提醒；
...
`@StepFinish` 总是返回 `Approve`。为续跑条件提供明确状态、最大次数和终止路径。
