...
| `feature-wrappers/` | 框架自带 feature 的薄包装（继承 + declareContinuity 叠加 Claw 协议，不复制实现）。当前含 `ControlledTodoFeature`（todo 中断控制 + 状态迁移）、`ContinuityAwareOpencodeBasic`（先读后写保护状态迁移）。消费方：编程小助手、official coder |
...
| `context-guard/` | 上下文过界检测核心 + 两个策略壳：`ContextGuardFeature`（交互式一次性拦截，编程小助手/agent-studio）、`ContextRotationTriggerFeature`（线程宿主自动接力触发器，official coder） | 活跃 |
