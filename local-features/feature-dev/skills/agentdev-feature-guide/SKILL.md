...
| 改写当前输入 | `@CallStart` + `ctx.agent.getUserInput()/setUserInput()` |
| 每个 ReAct step 前注入提醒 | `@StepStart` |
| 工具执行前校验或阻止 | `@ToolUse` |
| 工具完成后记录或同步 | `@ToolFinished` |
| 无工具调用时仍需继续或主动结束 call | `@StepFinish` |
| 按 call 结束原因处理结果 | `@CallFinish` + `ctx.finishReason` |
...
- 在继承带决策钩子的 Feature 时，覆盖原钩子方法；不要再声明第二个同生命周期决策钩子。
...
- 使用 dependencies、`getFeature()`、公开 API 或继承：读 [Feature 依赖与组合](references/runtime/composition.md)。
