...
1. 方法是否使用了 `@CallStart`、`@StepStart` 等装饰器。
2. 装饰器是否从 `agentdev` 导入。
3. Feature 是否在准备阶段完成 hook 收集。
4. 动态替换时旧实例是否仍在 registry。
5. 方法名是否在子类中被错误覆盖为非函数字段。

Feature 的一次性资源初始化使用 `onInitiate()`，不是普通的 Agent forward hook 同名方法。

## 决策 Hook 冲突

同一个 Feature 类中，`@ToolUse` 和 `@StepFinish` 各只能有一个。
...
继承已有决策 hook 时覆盖父方法，不添加第二个装饰器。

## 安全 Hook 抛错后工具仍执行

反向 hook 的异常会被记录并跳过，registry 会继续执行后续 hook；没有强决策时采用默认行为。

安全关键 `@ToolUse` 在方法内部捕获权限、策略和审计错误，并显式返回 `Decision.Deny`。不要用抛异常表达阻止。
...
- `@StepFinish` 无条件返回 `Decision.Approve`；
- 待办状态从未被清空；
