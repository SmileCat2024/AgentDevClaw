...
- `dependencies`：表达依赖意图。它不代替装配顺序或运行时校验；仍要在初始化时用 `ctx.getFeature()` 检查依赖是否存在。
...
使用 `agent.use(feature)` 将 Feature 加入 Agent。Agent 准备 Feature 时按注册顺序执行：

1. 收集所有 Feature 自带的 skills；
2. 调用每个 Feature 的 `getTools()` 并注册同步工具；
3. 调用 `getAsyncTools(ctx)` 并注册异步工具；
4. 调用 `onInitiate(ctx)`；
5. 收集该 Feature 的反向钩子；
6. 所有 Feature 完成后，调用 Agent 子类的 `onFeatureToolsReady()`。
