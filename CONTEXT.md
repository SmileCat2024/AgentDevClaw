...
**变更分类（changeKind）**:
一次消息更新相对于前端已持有转录的差异类别：`append`（尾部新增）、`tail`（末条改写）、
`rewrite`（中段替换或条数减少）。由运行时在消息推送时刻计算，前端按类别取增量并拼回
完整转录；分类错误通过长度校验降级全量拉兜底。

**探测（probe）**:
随 overview 响应下发的轻量变更线索（消息计数、变更分类、增量起点、假想全量字节数），
用于把全量转录轮询降为"未变化零取数、变化取增量"。是 HTTP 组装层产物，不属于
overview 快照语义。

**只读远程视图**:
远程运行时数据在本地页面的只读呈现。写操作被显式禁用并明示原因；不提供可输入但不生效的假交互，也不把用户输入暂存到本地队列等待重发。

**会话激活（session activation）**:
把已持久化的工作会话恢复为运行时实例的动作，总是伴随目标宿主上新运行时的启动。激活语义跨本地/远程统一：远程会话的激活发生在远程宿主，不引入本地专属的确认层或降级路径。
