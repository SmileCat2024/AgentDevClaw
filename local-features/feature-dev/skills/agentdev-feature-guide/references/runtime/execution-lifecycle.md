...
- Feature 注册顺序决定工具覆盖顺序、初始化顺序和 hooks 执行顺序。
...
- `@CallStart` 在系统提示词建立后、用户消息正式加入 Context 前执行。
- `@StepStart` 在本 step 的 LLM 调用前执行。
- `@ToolUse` 在工具调用前执行，并可以阻止调用。
- `@ToolFinished` 在成功、失败、被禁用或被拦截后都会收到结果通知。
- `@CallFinish` 同时覆盖成功、异常、中断和 continuation 等结束原因。
...
- `@CallStart`、`@CallFinish`、`@StepStart`、`@StepFinish`、`@ToolUse`、`@ToolFinished` 参与运行循环。

不要在 Feature 中仅声明一个普通 `onStepStart()` 方法并期待它被当作 Feature hook。Feature 的 call/step/tool 行为通过装饰器注册。
...
class SearchFeature implements AgentFeature {
  ...
  readonly dependencies = ['search-index'];
...
`dependencies` 表达关系，`ctx.getFeature()` 完成运行时解析。Agent 不会因为声明了 `dependencies` 就自动安装、排序或初始化缺失依赖。
...
如果父类已有一个 `@StepFinish` 方法，覆盖相同方法名并调用 `super`：
...
不要在子类再添加另一个 `@StepFinish`。Hooks registry 记录的是方法名，运行时会在子类实例上调用被覆盖的方法。
