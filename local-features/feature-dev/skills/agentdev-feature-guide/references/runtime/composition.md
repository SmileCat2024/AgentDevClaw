...
`dependencies` 是关系元数据。核心不会自动安装、排序或验证缺失依赖。按正确顺序注册，并在初始化时使用 `ctx.getFeature()` 解析。
...
依赖实例可能已经注册但尚未完成 `onInitiate()`。不要仅以实例存在判断可用；读取 readiness 或让 API 方法自行校验。
...
继承适合保留同一能力边界并增加小型策略。覆盖已装饰决策 hook 时复用同一方法名，不再次装饰。
