...
let IM_CHANNEL_ICONS = {
  ...
  rokid: '<svg class="im-channel-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round" xmlns="http://www.w3.org/2000/svg"><circle cx="6" cy="12" r="3"/><circle cx="18" cy="12" r="3"/><path d="M9 12h6"/><path d="M3 12V9a2 2 0 0 1 2-2h2"/><path d="M21 12V9a2 2 0 0 0-2-2h-2"/><path d="M3 12v3a2 2 0 0 0 2 2h1"/><path d="M21 12v3a2 2 0 0 1-2 2h-1"/></svg>',
};
  ...
  return {
    workspaceConfig: {
      ...
      channels: {
        ...
        rokid: {
          label: typeof channels?.rokid?.label === 'string' ? channels.rokid.label : '',
          note: typeof channels?.rokid?.note === 'string' ? channels.rokid.note : '',
        },
      },
    ...
    rokidConfig: {
      configured: !!raw?.rokidConfig?.configured,
      linkCode: typeof raw?.rokidConfig?.linkCode === 'string' ? raw.rokidConfig.linkCode : '',
      linkSecret: typeof raw?.rokidConfig?.linkSecret === 'string' ? raw.rokidConfig.linkSecret : '',
      wsUrl: typeof raw?.rokidConfig?.wsUrl === 'string' && raw.rokidConfig.wsUrl
        ? raw.rokidConfig.wsUrl
        : 'wss://rcs.rokid.com/claw/ws/link',
      sourcePath: typeof raw?.rokidConfig?.sourcePath === 'string' ? raw.rokidConfig.sourcePath : '',
    },
    binding: {
...
function renderLineConnectionSection(draft) {
  ...
  function renderLineRow(line) {
    ...
    let carrierOptions = [
      ...
      { value: 'feishu', label: '飞书', icon: IM_CHANNEL_ICONS.feishu },
      { value: 'wecom', label: '企业微信', icon: IM_CHANNEL_ICONS.wecom },
      { value: 'rokid', label: 'Rokid 眼镜', icon: IM_CHANNEL_ICONS.rokid },
    ].filter(function(o) {
...
function renderIMWorkspaceConfigEditor(block) {
  ...
  const rokidConfigured = !!(draft.rokidConfig?.configured || (draft.rokidConfig?.linkCode && draft.rokidConfig?.linkSecret));
  ...
  return [
    ...
    sessionsHtml,
    '</section>',
    renderIMChannelConfigDialog(draft, fields, qqConfigured, weixinConfigured, feishuConfigured, wecomConfigured, rokidConfigured, weixinBadge, weixinActionsHtml),
  ].join('');
}

function renderIMChannelConfigDialog(draft, fields, qqConfigured, weixinConfigured, feishuConfigured, wecomConfigured, rokidConfigured, weixinBadge, weixinActionsHtml) {
  ...
  if (window._imChannelDetailId) {
      ...
      : isWeixin ? (isZh ? '微信配置' : 'WeChat Config')
        : isFeishu ? (isZh ? '飞书配置' : 'Feishu Config')
          : isWecom ? (isZh ? '企业微信配置' : 'WeCom Config')
            : isRokid ? (isZh ? 'Rokid 眼镜配置' : 'Rokid Glasses Config')
              : ch;
      ...
      : isWeixin ? (isZh ? '配置微信账号绑定与扫码登录' : 'Configure WeChat binding & QR login')
        : isFeishu ? (isZh ? '配置飞书自建应用凭据' : 'Configure Feishu app credentials')
          : isWecom ? (isZh ? '配置企业微信智能机器人凭据' : 'Configure WeCom smart bot credentials')
            : isRokid ? (isZh ? '配置 Rokid 眼镜配对码' : 'Configure Rokid glasses pairing code')
              : '';
    ...
    if (isQQ) {
      ...
      const rokidFields = [
        { name: 'linkCode', type: 'text', label: isZh ? '配对码 (Link Code)' : 'Link Code' },
        { name: 'linkSecret', type: 'password', label: isZh ? '配对密钥 (Link Secret)' : 'Link Secret' },
      ];
      bodyHtml = '<div class="workspace-config-grid">'
        + rokidFields.map((field) => renderQQBotConfigField(field, draft.rokidConfig || {}, 'window.updateIMRokidConfigDraft')).join('')
        + '</div>'
        + '<div class="workspace-form-note" style="margin-top:12px;">'
        + (isZh
          ? '在 Rokid 眼镜的「开放 Claw」配对页面获取配对码与密钥。保存后切换当前渠道为 Rokid 眼镜即可建立连接。'
          : 'Get Link Code & Secret from the "Open Claw" pairing page on Rokid glasses. Save, then switch the current channel to Rokid to connect.')
        + '</div>';
    }
  ...
  const secondaryChannels = [
    {
      id: 'rokid',
      icon: IM_CHANNEL_ICONS.rokid,
      name: isZh ? 'Rokid 眼镜' : 'Rokid Glasses',
      desc: isZh ? 'Rokid AR 眼镜配对码与 WebSocket 连接' : 'Rokid AR glasses pairing & WebSocket link',
      configured: rokidConfigured,
      badgeText: rokidConfigured ? (isZh ? '已配置' : 'Ready') : (isZh ? '未完成' : 'Incomplete'),
    },
  ];
  ...
  const primaryRows = primaryChannels.map(renderChannelRow).join('');
  const hasSecondary = secondaryChannels.length > 0;
  const secondaryExpanded = !!window._imMoreChannelsOpen;
  const secondaryRows = hasSecondary ? secondaryChannels.map(renderChannelRow).join('') : '';

  // Always render secondary rows when there is at least one configured secondary
  // channel — otherwise users would lose visibility on a working channel.
  const anySecondaryConfigured = secondaryChannels.some(ch => ch.configured);
  const showSecondaryExpanded = secondaryExpanded || anySecondaryConfigured;

  const moreHeaderHtml = hasSecondary ? (
    '<div class="im-ch-more-toggle" onclick="window.toggleIMMoreChannels()">'
    + '<span class="im-ch-more-arrow' + (showSecondaryExpanded ? ' expanded' : '') + '">▶</span>'
    + '<span class="im-ch-more-label">' + (isZh ? '更多渠道' : 'More channels') + '</span>'
    + '<span class="im-ch-more-count">' + secondaryChannels.length + '</span>'
    + '</div>'
  ) : '';
  const secondarySectionHtml = hasSecondary ? (
    '<div class="im-ch-more-section' + (showSecondaryExpanded ? ' expanded' : '') + '">'
    + secondaryRows
    + '</div>'
  ) : '';
