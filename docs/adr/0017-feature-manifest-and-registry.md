...
**四次修订（来源分类以装配事实为准）**：seed 的 provenance→group 静态映射把官方选装经 `$mount` 挂载的 feature（如 playwright-shell）归入 bundled，与挂载管理页按装配事实（base vs `$mount`）的分类不一致——同一配置页显示"已安装 1"，面板"已安装 0"。修正为：面板来源分类以当前身份的 `$mount` 装配事实（`/api/feature-store/overview` 的 mounts）优先判定，seed 静态映射仅在装配事实不可用时兜底（远程会话、非编程小助手宿主）；分页器文案"官方内置"改"默认内置"，对齐 base 装配语义。

| 维度 | 回答的问题 | 值形态 | 用途 |
|---|---|---|---|
| **功能类型 `type`** | 用户装它是为了什么？ | 单值 | 面板主分组轴 |
| **来源** | 谁提供的？ | 单值 | 面板右上角过滤器（全部 / 默认内置 / 已安装） |
